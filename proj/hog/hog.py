...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        ...
        if point == 1:
            is_roll_one = True
        sum += point
    return 1 if is_roll_one else sum
    '''
    ...
    while i < num_rolls:
        ...
        sum += point
        i += 1
    return sum if not is_roll_one else 1
    '''

    # 集合版本
    point = [dice() for _ in range(num_rolls)]
    return 1 if 1 in point else sum(point)
    # END PROBLEM 1
...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return 1 + 2 * abs(opponent_score // 10 % 10 - opponent_score % 10)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for score0 in range(goal):
        for score1 in range(goal):
            if test != strategy(score0, score1):
                return False
    return True
    '''

    # create list to compare
    return False not in [False not in [test == strategy(score0, score1) for score1 in range(goal)] for score0 in range(goal)]
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    aver = make_averaged(roll_dice, total_samples)
    max_index, max_value = 1, aver(1, dice)
    for i in range(2, 11):
        value = aver(i, dice)
        if value > max_value:
            max_index, max_value = i, value
    return max_index
    # END PROBLEM 9
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    return 0 if tail_point >= threshold else num_rolls
    # END PROBLEM 10
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tail_point = tail_points(opponent_score)
    if perfect_square(tail_point + score):
        tail_point = next_perfect_square(score + tail_point) - score 
    return 0 if tail_point > threshold else num_rolls
    # END PROBLEM 11


THRESHOLD = max_scoring_num_rolls()
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    final_score = score + tail_points(opponent_score)
    if final_score >= GOAL or (perfect_square(final_score) and next_perfect_square(final_score) >= 100):
        return 0
    return square_strategy(score, opponent_score, THRESHOLD / 2, num_rolls = 6)    
