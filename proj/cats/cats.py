...
def fastest_words(match):
    ...
    # 单个添加, 思路不清
    player = [[] for _ in player_indices]
    for word_i in word_indices:
        min_player_i, min_player_val = 0, time(match, 0, word_i)
        for player_i in player_indices:
            # match 写成 main | 变量名的错误
            cur_player_val = time(match, player_i, word_i)
            if cur_player_val < min_player_val:
                min_player_i, min_player_val = player_i, cur_player_val
        # 列表添加元素, 而不是列表, 确认类型的错误
        player[min_player_i] += [get_word(match, word_i)]
    return player
    # END PROBLEM 10
