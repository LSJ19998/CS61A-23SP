...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        return self.cents + (Mint.present_year - self.year - 50 if Mint.present_year - self.year > 50 else 0)
...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.dollar < self.price:
            return f'Please add ${self.price - self.dollar} more funds.'
        if self.dollar >= self.price:
            self.number -= 1
            charge = self.dollar - self.price
            self.dollar = 0
            if charge == 0:
    ...
    def add_funds(self, num):
        ...
        self.dollar += num
        return f"Current balance: ${self.dollar}"
...
class Player:
    ...
    def speech(self, other):
        "*** YOUR CODE HERE ***"
        self.votes, self.popularity = self.votes + \
            self.popularity // 10, self.popularity + self.popularity // 10
...
class VirFib():
    ...
    def next(self):
        "*** YOUR CODE HERE ***"
        # 实例的赋值 | 从0开始,然后到其他, 从最开始的0就可以赋值一个prev
        if self.value == 0:
            cur = VirFib(1)
        else:
            cur = VirFib(self.value + self.prev)
        cur.prev = self.value
        return cur
